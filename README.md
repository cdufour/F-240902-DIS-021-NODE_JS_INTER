# Nodejs

## Liens utiles
- [Apprendre nodejs](https://nodejs.dev/fr/learn/introduction-to-nodejs/)
- [Mime types](https://www.freeformatter.com/mime-types-list.html)
- [12factor](https://12factor.net/)
- [Méthodes HTTP](https://developer.mozilla.org/fr/docs/Web/HTTP/Methods)
- [6 frameworks NodeJS à connaître en 2023](https://www.codeur.com/blog/frameworks-nodejs-a-connaitre/)
- [Top Express.js template engines for dynamic HTML pages](https://blog.logrocket.com/top-express-js-template-engines-for-dynamic-html-pages/)
- [JWT](https://jwt.io/)
- [Intro To JavaScript Unit Testing With Mocha JS & Chai | youtube | EN](https://youtu.be/MLTRHc5dk6s)
- [Testing a REST API in Node JS with Express using Mocha and Chai | youtube | EN](https://youtu.be/I4BZQr-5mBY)
- [Node.js Tutorial - 61 - Cluster Module  | youtube | EN](https://youtu.be/SHR-KmfRIsU)
- [Understanding Streams | Understanding Node.js Core Concepts | youtube | EN](https://youtu.be/e5E8HHEYRNI)
- [Understanding Buffers | Understanding Node.js Core Concepts | youtube | EN](https://youtu.be/QZIeZM-yXXU)
- [Connecting NodeJS with MongoDB | Mongoose + Express | youtube | EN | 19 min](https://youtu.be/xrglM8U0Zv8?si=bs0VyVqh_DAnGAjl)